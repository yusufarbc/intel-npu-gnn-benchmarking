...
\label{tab:int8}
\end{center}
\footnotesize{Speedup $>$ 1.0 indicates INT8 faster. GAT and GATv2 excluded (INT8 compilation failed). EfficientNet-B0 also fails INT8. Vision models omitted due to evidence of NPU$\rightarrow$CPU fallback under INT8 (see text).}
\end{table}
...
The INT8 results for dense vision models require careful interpretation. MobileNetV2 INT8 reports 5.38\,ms on NPU versus 1.90\,ms FP32 (0.35$\times$). However, the INT8 CPU latency is 5.46\,ms---nearly identical to the reported NPU value---suggesting that the INT8 variant silently falls back to CPU execution. This pattern repeats for ResNet50 (4.98\,ms NPU INT8 vs.\ 18.50\,ms CPU INT8, though here the disparity suggests partial NPU execution) and BERT-Tiny (8.02\,ms NPU INT8 vs.\ 8.13\,ms CPU INT8). EfficientNet-B0 INT8 compilation fails entirely (a .failed marker is produced). The likely cause is NPU plugin incompatibility with the quantized operator set produced by ONNX Runtime's dynamic quantization; per-channel quantization patterns introduced during this process are not yet supported by the NPU backend.
...
\textit{Statistical reporting.} Reported latencies are arithmetic means over 100 iterations $\times$ 3 independent repeats; per-iteration standard deviations are visualized as error bars in Fig.~\ref{fig:fig1}, but formal confidence intervals and hypothesis tests are not provided. Small differences between backends---such as the $\sim$6\% CPU--NPU latency gap for most GNNs---are not assessed for statistical significance. Given the run-to-run variability inherent in a shared-resource laptop environment (thermal throttling, OS scheduler preemption, DVFS transitions), some of the reported device rankings may not be statistically distinguishable. Future work should incorporate 95\% confidence intervals and paired $t$-tests or non-parametric alternatives where normality assumptions are violated.

\textit{Power measurement coverage.} The SoCWatch power data (Table~\ref{tab:energy}) cover only two of 14 models (GCN and MPNN) and two of three backends (CPU and GPU). The ``9.1--12.5\,W'' power range cited throughout this paper is based on these four configurations and should not be interpreted as a universal characterization of all GNN workloads on this platform. NPU-specific power isolation was not available through the PMT interface, and the SoCWatch Python API was blocked by Hyper-V/VBS---a limitation circumvented through the CLI-based PMT path only for CPU/GPU package-level data. Extending energy analysis to all 14 models and to the NPU backend requires either a future SoCWatch release with NPU power rail support or external power instrumentation.
