...
\begin{abstract}
Neural Processing Units (NPUs) in consumer processors such as Intel Core Ultra (Meteor Lake) promise energy-efficient inference at the edge. This paper measures 14 models---Graph Neural Networks (GNNs), convolutional networks, and vision transformers---on the CPU, integrated GPU (iGPU), and NPU of a Core Ultra 5 125H platform running OpenVINO, across three Open Graph Benchmark datasets (ogbn-arxiv, ogbn-proteins, ogbn-products). The NPU delivers strong FP32 performance for dense vision models (MobileNetV2: 1.90\,ms, ResNet50: 3.94\,ms, ViT-Tiny: 9.10\,ms, 4.5--11.4$\times$ over CPU), but offers negligible to negative returns for GNN workloads---the iGPU consistently delivers the lowest GNN latency across all tested architectures. For most GNNs, NPU latency remains within 6\% of CPU; SGC INT8 exhibits a 2.2$\times$ regression over FP32 (173.90\,ms vs.\ 78.59\,ms), MPNN falls back entirely to CPU, and three architectures (GAT, GATv2, EfficientNet-B0) fail INT8 compilation. NPU power isolation was unavailable through the SoCWatch PMT interface; CPU and GPU measurements show comparable package power (9.1--12.5\,W), with INT8 offering at best 18\% energy reduction on CPU. These findings indicate that while the Meteor Lake NPU is a capable accelerator for dense vision inference, the current OpenVINO toolchain and hardware architecture are not yet mature for sparse graph workloads---particularly under quantization. The benchmarking suite and processed artifacts are released at \url{https://yusufarbc.github.io/intel-npu-gnn-benchmarking/}.
\end{abstract}
...
These results may appear contradictory at first glance: INT8 quantization is widely promoted as a key NPU advantage, yet many configurations show degradation rather than improvement. The root causes are discussed in Section~V-A.
...
Figs.~\ref{fig:fig7} and~\ref{fig:fig6} examine the relationship between graph structure and NPU latency from two complementary angles: density (edges per node) across the three OGB datasets (spanning 6.9--451.7\,edges/node) and node/edge count on ogbn-arxiv. Both analyses converge on the same finding: NPU latency is nearly constant regardless of graph density ($r=-0.00$, $p=0.993$) or graph size. This behavior arises because ONNX Runtime compiles the NPU model with statically-shaped tensors; the compiled execution plan processes the fixed-size graph representation at constant throughput. Unlike CPU and GPU, where latency scales with graph size, the NPU does not realize efficiency benefits for sparser subgraphs of the same model---a trade-off that favors latency predictability over proportional savings.
...
These measurements, while limited to two GNN models and two backends, provide quantitative evidence that for sparse GNN workloads on Meteor Lake, the CPU and GPU backends operate within overlapping power-latency trade-off regions, and that INT8 energy savings are model-dependent and far from guaranteed. The NPU's lower TDP suggests potential energy savings for dense vision models where it achieves significant latency reductions, but direct NPU power measurement requires either a future SoCWatch release with NPU power rail isolation or external instrumentation. The full latency dataset is included in the released artifact repository.
...
What further distinguishes the Meteor Lake NPU from other accelerators is the nature of the failure mode: rather than rejecting unsupported models at compile time, the OpenVINO toolchain silently falls back to CPU for unsupported quantized operators. This behavior---observed for MobileNetV2, ResNet50, and BERT-Tiny INT8 in our measurements---can produce misleading latency numbers unless practitioners actively verify CPU fallback ratios. As NPU compilers evolve toward explicit mixed-precision support, the fraction of GNN operators that can benefit from reduced precision should increase, but the fundamental memory-bandwidth bottleneck will persist for any architecture that does not incorporate sparse-dataflow acceleration.
...
The iGPU's consistent latency advantage over both CPU and NPU for GNN workloads stems from three architectural factors: higher effective memory bandwidth (dedicated 128-bit LPDDR5x controller vs. the NPU's shared SoC fabric), larger on-die cache (4--8\,MB L2 vs. the NPU's small explicitly-managed SRAM), and more aggressive kernel fusion under the mature OpenVINO GPU plugin (1.1--1.25$\times$ optimization speedup for GNNs vs.\ 1.0--1.05$\times$ on NPU, Section~IV-E). Since GNN execution is memory-bound (Section~IV-E, Fig.~\ref{fig:fig5b}), bandwidth and cache capacity directly translate to lower latency. These hardware-level constraints align with algorithmic evidence that GNNs are not always the optimal choice for sparse graph problems \cite{Bayraktar2026}.

\subsection{Comparison with Prior Work}
Custom GNN accelerators such as HyGCN \cite{9065592} ($10^{3}\times$ over CPU), EnGN \cite{9161360} ($1.8\times10^{3}\times$ over CPU), and GRIP \cite{9858684} ($17\times$ over CPU) demonstrate the potential of hardware-native sparse engines and dedicated memory hierarchies for graph workloads, but rely on custom silicon unavailable in commodity edge platforms. The Meteor Lake NPU provides negligible acceleration for GNNs (0.9--1.2$\times$ over CPU). This gap reflects fundamentally different design goals: ASIC accelerators optimize for graph-specific dataflows, while the consumer NPU targets general-purpose dense inference. These findings are consistent with the broader edge AI landscape \cite{SINGH202371}.
...
The root cause is not a single hardware defect but a systemic architectural mismatch: the NPU's streaming-dataflow design and static-shape compilation model are optimized for dense, regular computation graphs, while GNNs are dominated by sparse, irregular memory access patterns where reducing arithmetic precision does not reduce DRAM traffic. This pattern suggests that GNN quantization challenges stem from architectural constraints common to streaming-dataflow accelerators, rather than being vendor-specific.
