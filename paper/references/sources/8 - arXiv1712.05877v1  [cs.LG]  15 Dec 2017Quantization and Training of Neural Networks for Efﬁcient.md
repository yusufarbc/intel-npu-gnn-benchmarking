# arXiv1712.05877v1  [cs.LG]  15 Dec 2017Quantization and Training of Neural Networks for Efﬁcient

## Page 1

arXiv:1712.05877v1  [cs.LG]  15 Dec 2017Quantization and Training of Neural Networks for Efﬁcient
Integer-Arithmetic-Only Inference
Benoit Jacob Skirmantas Kligys Bo Chen Menglong Zhu
Matthew Tang Andrew Howard Hartwig Adam Dmitry Kalenichenk o
...
signs novel network architectures that exploit computatio n
/ memory efﬁcient operations. The second category quan-tizes the weights and / or activations of a CNN from 32
bit ﬂoating point into lower bit-depth representations. Th is
...
obtain sizable compression of these architectures, reduci ng
...
ﬁcient at trading off latency with accuracy, e.g. MobileNet s.
...
vations are quantized. Notably, these approaches rarely pr o-
...
are represented as 8-bit integers according to equation 1. The convolution involves 8-bit integer operands and a 32-b it integer accumulator.
...
with simulated quantization of the convolution layer. All v ariables and computations are carried out using 32-bit ﬂoat ing-point arithmetic.
Weight quantization (“wt quant”) and activation quantizat ion (“act quant”) nodes are injected into the computation gr aph to simulate the
effects of quantization of the variables (section 3). The resultant graph approximates the integer-arithmeti c-only computation graph in panel
...
the fast integer-arithmetic circuits in common CPUs to deli ver an improved latency-vs-accuracy tradeoff (section 4). The ﬁgure compares
...
tions and additions can be implemented by efﬁcient bit-shif t
...
low-precision ﬁxed-point arithmetic to accelerate the tra in-
...
ing inference and ﬂoating-point arithmetic during trainin g,
...
both integer-arithmetic inference and ﬂoating-point trai n-
ing.
...
treatment of some of the topics developed in this section, an d for its self-
contained example code.

## Page 3

A basic requirement of our quantization scheme is that it
permits efﬁcient implementation of all arithmetic using on ly
...
Some arrays, typically bias vectors, are quantized as 32-bi t
...
ral network operators often requires zero-padding of array s
around boundaries.
Our discussion so far is summarized in the following
quantized buffer data structure3, with one instance of such a
buffer existing for each activations array and weights arra y
...
to translate real-numbers computation into quantized-val ues
...
in the next subsection, this data structure, which still con tains a ﬂoating-
point quantity, does not appear in the actual quantized on-d evice inference
...
shift, albeit one that needs to have correct round-to-neare st
...
into 16-bit integers, we ﬁrst notice that by distributing th e
...
of arbitrary zero-points for anything but the smallest valu es
...
plicitly deﬁne the data types of all quantities involved, an d
modify the quantized matrix multiplication ( 7) to merge
the bias-addition and activation function evaluation dire ctly
...
the granularity of fused operators in inference code (takin g
...
of an int32 bias into this int32 accumulator, the bias-vecto r
...
in the bias-vector tends to act as an overall bias (i.e. an err or
...
in appendix A.1and we do not currently fuse them into such
...
activation. In practice, the quantized training process (s ec-
tion3) tends to learn to make use of the whole output uint8
...
6The quantization of bias-vectors discussed here is impleme nted here
...
weights (sometimes with additional post-quantization tra in-
ing for ﬁne-tuning). We found that this approach works
sufﬁciently well for large models with considerable repre-
sentational capacity, but leads to signiﬁcant accuracy dro ps
...
fects in the forward pass of training. Backpropagation stil l
...
performed by applying point-wise the quantization functio n
qdeﬁned as follows:
...
bit quantization.3.1. Learning quantization ranges
...
after quantization. As a result, the learned quantization p a-
...
represented as 32-bit integers in the inference process, wi th
...
fromtf.contrib.quantize \
importquantize_graph as qg
g = tf.Graph()
withg.as_default():
output = ...
total_loss = ...
...
is additional complexity: the training graph contains batc h
...
EMA(σ2
B)+ε. (14)
Hereγis the batch normalization’s scale parameter,
EMA(σ2
B)is the moving average estimate of the variance
of convolution results across the batch, and εis just a small
...
for a batch-normalized convolutional layer, the correspon d-
ing inference graph (ﬁgure C.6), the training graph after
batch-norm folding (ﬁgure C.7) and the training graph af-
ter both folding and quantization (ﬁgure C.8).ResNet depth 50 100 150
...
Table 4.2: ResNet on ImageNet: Accuracy under var-
...
and the other illustrating the improved latency-vs-accura cy
...
bits, respectively. 7-bit quantization is obtained by sett ing
...
from a possibly larger interval, with different ranges in di f-
...
state-of-the-art tradeoff between on-device latency and I m-
ageNet classiﬁcation accuracy. In this section we demon-
strate how integer-only quantization can further improve t he
...
quantization literature focuses on minimizing accuracy lo ss
...
ware. Floating-point computation is better optimized in th e
...
tween ﬂoating-point and integer-quantized models. Latenc y
...
As indicated by tables 4.5and4.6, quantization provides
...
25% face detector to run in real-time ( 1K/28≈36fps) on
a single big core, whereas the ﬂoating-point model remains
slower than real-time ( 1K/44≈23fps).
We additionally examine the effect of multi-threading on
the latency of quantized models. Table 4.6shows a1.5to
2.2×) speedup when using 4cores. The speedup ratios are
...
Figure 4.3shows the latency-vs-accuracy tradeoff of face
attribute classiﬁcation on the Qualcomm Snapdragon 821.DM type Precision Recall
100% ﬂoats 68% 76%
8 bits 66% 75%
50% ﬂoats 65% 70%
8 bits 62% 70%
...
ues where an IOU of xbetween the groundtruth and pre-
dicted windows is considered a correct detection, for xin
{0.5,0.55,...,0.95}.
...
Table 4.7: Face attributes: relative average category prec i-
...
training with varying weight and activation quantization b it
depths. The degradation in average precision for binary at-
tributes and age precision relative to the ﬂoating-point ba se-
line are shown in Tables 4.7and4.8, respectively. The ta-
...
ment advances the state-of-the-art tradeoff between laten cy
...
time and low-end phone market.References
...
[7] S. Gupta, A. Agrawal, K. Gopalakrishnan, and P. Narayana n.
...
pressing deep neural network with pruning, trained quantiz a-
...
cient convolutional neural networks for mobile vision appl i-
...
Speed/accuracy trade-offs for modern convolutional objec t
...
[17] S. Ioffe and C. Szegedy. Batch normalization: Accelera ting
...
sorﬂow quantized training support.
https://github.com/tensorflow/tensorflow/tree/maste r/tensorflow/contrib/quantize .
5,6
[20] A. Krizhevsky, I. Sutskever, and G. E. Hinton. Imagenet
classiﬁcation with deep convolutional neural networks. In
Advances in neural information processing systems , pages
1097–1105, 2012. 1
[21] C. Leng, H. Li, S. Zhu, and R. Jin. Extremely low bit neura l
...
[27] M. Rastegari, V . Ordonez, J. Redmon, and A. Farhadi. Xno r-
net: Imagenet classiﬁcation using binary convolutional ne u-
...
pages 1–9, 2015. 1
[30] C. Szegedy, V . Vanhoucke, S. Ioffe, J. Shlens, and Z. Woj na.
...
ume 1, page 4, 2011. 2[32] X. Zhang, X. Zhou, M. Lin, and J. Sun. Shufﬂenet: An
extremely efﬁcient convolutional neural network for mobil e
...
point multiplication by the multiplier M=S1/S2similar
...
is very important to use the correctly-rounding instructio n
SQRDMULH and not SQDMULH10.
The rounding-to-nearest right-shifts referenced in sec-
tion2.2do not map exactly to any ARM NEON instruction.
7Pure-arithmetic, SIMD-ready, branch-free, ﬁxed-point im plementa-
tions of at least tanh and the logistic functions are given in gemmlowp
[18]’sﬁxedpoint directory , with specializations for NEON and SSE in-
...
such ﬁxed-point multiplications, and ordinary (non-satur ating) integer ad-
ditions. We have no use for general saturated arithmetic.The problem is that the “rounding right shift” instruction,
...
the result will be −1whereas it should be −2with “round
...
correct round-to-nearest right-shift can still be impleme nted
using RSHL but with suitable ﬁx-up arithmetic around it11.
For efﬁcient NEON implementation of the matrix mul-
tiplication’s core accumulation, we use the following tric k.
...
fore always less than 214in absolute value. Hence, ( B.1)
can accumulate twoproducts on a local int16 accumulator
...
sized preserving aspect ratio so that the smallest side of th e
image is256. Then the center 224×224patch is cropped
and the means are subtracted for each of the RGB channels.
Optimization . We use the momentum optimizer from
TensorFlow [ 1] with momentum 0.9and a batch size of 32.
The learning rate starts from 10−5and decays in a staircase
fashion by 0.1for every 30epochs. Activation quantization
is delayed for 500,000steps for reasons discussed in section
3. Training uses 50workers asynchronously, and stops after
validation accuracy plateaus, normally after 100epochs.
...
the choice of L8R8WithLhsNonzeroBitDepthParams atthis line ).
...
weights
input+
biasesReLU6act quantoutput
Figure C.2: Simple graph: quantized
D.2. Inception protocol
All results in table 4.3were obtained after training for
approximately 10million steps, with batches of 32samples,
using50distributed workers, asynchronously. Training data
were ImageNet 2012 299×299images with labels. Image
augmentation consisted of: random crops, random horizon-
tal ﬂips, and random color distortion. The optimizer used
was RMSProp with learning rate starting at 0.045and de-conv
weights
input+
biases+ReLU6output
Figure C.3: Layer with a bypass connection: original
caying exponentially and stepwise with factor 0.94after ev-
ery2epochs. Other RMSProp parameters were: 0.9mo-
mentum, 0.9decay,1.0epsilon term. Trained parameters
...
starts from 4×10−3and decays in a staircase fashion by a
factor of0.1for every 100epochs. Activation quantization
is delayed for 500,000steps for reasons discussed in section
3. Training uses 20workers asynchronously, and stops after
validation accuracy plateaus, normally after approximate ly
6million steps.
...
320×320pixels for face detection and 128×128pixels for
face attribute classiﬁcation. The resulting crops are ﬂipp ed
...
weights
input+
biasesconv quant+ReLU6act quantoutput
...
learning rate starts from 4×10−3and decays in a stair-
case fashion by a factor of 0.1for every 100epochs. Ac-
tivation quantization is delayed for 500,000steps for rea-
sons discussed in section 3. Training uses 20workers asyn-
chronously, and stops after validation accuracy plateaus,
normally after approximately 3million steps.
...
and a constant learning rate of 0.1. Training uses 12work-
ers asynchronously, and stops at 20million steps.
Latency Measurements . We created a binary that runs
the face detection and face attributes classiﬁcation model s
repeatedly on random inputs for 100seconds. We pushed
...
ported the average runtime of the face detector model onconv
weights
inputmomentsMAµ,σγ(x−µ)/σ+β
γ βReLU6output
...
inference graph
320×320inputs, and of the face attributes classiﬁer model
on128×128inputs.
...
conv
weights
inputmomentsMAµ,σwγ/σ
γconv fold
β−γµ/σ
β+ReLU6output
Figure C.7: Convolutional layer with batch normalization:
training graph, foldedconv
weights
inputmomentsMAµ,σwγ/σ
γwt quantconv fold
β−γµ/σ
β+ReLU6act quantoutput
